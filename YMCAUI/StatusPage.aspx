<%@ Page Language="vb" Trace="false" AutoEventWireup="true" %>
...
<%--CodeBehind="StatusPage.aspx.vb" Inherits="YMCAUI.StatusPage"--%> 
...
<%@ Import Namespace="YMCARET.YmcaBusinessObject" %>
...
<script runat="server" >
    ...
    Private Function createConfigList() As object
    ...
    Private Function ReadAppsettingFromWebConfig()
        ...
        Try
            ...
            If g_dbConn = True Then 'Database connection check
                ...
                ds_Status = checkEmailStatus()
                CheckPrinterAndPaperTypeSetting(ds_Status)
            End If
            '---------------------------------------------------
    ...
    Sub CheckPrinterAndPaperTypeSetting(ByVal paramDs_Status As DataSet)
        Dim isPaperTypeMatdhed As Boolean = False
        Try
            If Not paramDs_Status Is Nothing AndAlso (Not paramDs_Status.Tables("PrinterSettingList") Is Nothing _
                  AndAlso paramDs_Status.Tables("PrinterSettingList").Rows.Count > 0) Then
                Dim dr As DataRow
            
                For Each dr In paramDs_Status.Tables("PrinterSettingList").Rows
                    isPaperTypeMatdhed = GetPrinterProperties(dr.Item("PaperType"), dr.Item("PrinterName"))
                    If isPaperTypeMatdhed Then
                        AddToErrorMesageList("Printer", "Paper type """ + dr.Item("PaperType") + " is found for printer " + dr.Item("PrinterName"))
                    Else
                        AddToErrorMesageList("Printer", "Paper type """ + dr.Item("PaperType") + " is not found for printer " + dr.Item("PrinterName"),True)
                    End If
                Next
            Else
                AddToErrorMesageList("Printer", "No printer list found in database.", True)
            End If
    ...
    Private Function GetPrinterProperties(ByVal paperType As String, ByVal printerName As String) As Boolean
        ...
        Try
            ...
            'Response.Write(String.Format("<h1>Paper Types for printer {0}</h1>", printerName))
            ...
            For Each paperSource In printerSettings.PaperSources
                ...
                If paperType.Trim.ToUpper() = paperSource.SourceName.ToString().ToUpper() Then
                    isFound = True
                    Exit For
                End If
    ...
    Private Function checkEmailStatus() As DataSet
        ...
        Try
            ...
            If Not (l_DataSet Is Nothing) Then
                For Each dt As DataTable In l_DataSet.Tables
                    If dt.Rows.Count > 0 Then
                        dt.TableName = dt.Rows(0)("TableName").ToString.Trim()
                    End If
                Next
                
                If l_DataSet.Tables("EmailStatus").Rows.Count > 0 Then
...
<body>
    ...
    <div class="Div_Center">
        <table class="Table_WithoutBorder" cellspacing="0" cellpadding="0" width="700">
            ...
            <!-- <tr>  <td align =left >&nbsp;&nbsp; <asp:Label ID="Label1" runat="server" Text="Key"></asp:Label></td>
    <td align =left ><asp:Label ID="lblMessage" runat="server" Text="Message"></asp:Label></td>
    </tr>-->
            <tr>
                <td>
                    &nbsp;
                ...
                <table class="Table_WithBorder" width="740">
                    ...
                    <tr>
                        <td runat="server" id="trUserid" height="10">
                            
                        </td>
                    </tr>
                    <tr>
                        <td>
                            <div style=" width: 700px; text-align: Center; font-weight: normal">
                                <asp:GridView ID="gdvKeyStatus" runat="server" CssClass="DataGrid_Grid" AutoGenerateColumns="false">
                                    <Columns>
                                        <asp:BoundField HeaderText="Key" DataField="KeyName" />
                                        <asp:BoundField HeaderText="Message" DataField="Message" />
                                        <asp:TemplateField HeaderText="IsFailed" HeaderStyle-HorizontalAlign ="Center"  >
                                            <ItemTemplate >
                                                <asp:Image ID="imgStatus" runat="server"  ImageUrl='<%# GetImage(Convert.ToBoolean(DataBinder.Eval(Container.DataItem, "IsFail"))) %>'>
                                                </asp:Image>
                                            </ItemTemplate>
                                        </asp:TemplateField>
                                    </Columns>
