...
//	last updated 18 de Agosto de 2005
...
{
	this.majorVersion=3
	this.minorVersion=2.0
	this.newCalendar=new Function("CalName","return(PopCalGetCalendarInstance(CalName))")
}

function PopCalGetCalendarInstance(CalName)
{
	var _id=PopCalGetCalendarId(CalName)
	if(_id==-1)
	{
		_id=objPopCalList.length
	}
	objPopCal=new PoPCalCreateCalendarInstance()
	if(objPopCal.dom)
	{
		objPopCal.id=_id
		objPopCal.calendarName=CalName
...
function PopCalGetCalendarId(CalName)
{
	for(var i=0;i<objPopCalList.length;i++)
	{
		if(objPopCalList[i].calendarName==CalName)
		{
			return(objPopCalList[i].id)
		}
	}
	return(-1)
}
